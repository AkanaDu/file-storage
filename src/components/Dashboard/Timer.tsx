 ...
 * @LastEditTime: 2024-11-19 14:16:29
  ...
  setup() {
    ...
    const refreshTime = () => {
      ...
      timer.value = setInterval(() => {
        const now = new Date()
        date.value = moment(now).format('YYYY年MM月DD日')
        time.value = moment(now).format('HH:mm:ss')
      }, 1000)
    }
    return () => (
      <div class="time absolute flex justify-end items-center space-x-2 z-50 left-2 top-2 text-base">
